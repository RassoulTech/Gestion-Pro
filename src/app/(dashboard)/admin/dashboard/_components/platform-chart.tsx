...
export function PlatformChart({ revenuTotal, revenuMensuel, data = [] }: PlatformChartProps) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-zinc-200/60 bg-white/60 p-6 shadow-lg shadow-zinc-100/50 backdrop-blur-xl transition-all duration-300 hover:shadow-xl dark:border-white/10 dark:bg-zinc-900/50 dark:shadow-none">
      ...
      <div className="absolute -right-32 -top-32 -z-10 h-64 w-64 rounded-full bg-orange-500/10 blur-3xl" />
      ...
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-orange-500/10 border border-orange-500/20 px-2 py-0.5 text-[10px] font-black text-orange-600 dark:text-orange-400">
              <TrendingUp className="h-3 w-3" /> {growth >= 0 ? `+${growth}` : growth}% (fin de période)
            </span>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2 rounded-lg border border-zinc-100 bg-zinc-50/50 px-3 py-1.5 dark:border-zinc-800/60 dark:bg-zinc-900/50">
            <span className="h-2 w-2 rounded-full bg-orange-500 shadow-[0_0_8px_#f59e0b]" />
            <span className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">
              Revenus : {formatCurrency(revenuMensuel)} sur la période
            </span>
          </div>
          <div className="flex items-center gap-2 rounded-lg border border-zinc-100 bg-zinc-50/50 px-3 py-1.5 dark:border-zinc-800/60 dark:bg-zinc-900/50">
