...
export function LogsClientView({ initialLogs, total }: LogsClientViewProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative border-l border-slate-200/60 ml-4 pl-8 space-y-6 dark:border-slate-800/80">
        <AnimatePresence mode="wait">
          {filteredLogs.length === 0 ? (
            ...
            >
              ...
              <h4 className="text-lg font-extrabold text-slate-900 dark:text-slate-50">Aucun log trouvé</h4>
            ...
            filteredLogs.map((log, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`absolute -left-[49px] top-1 flex h-10 w-10 items-center justify-center rounded-xl border shadow-md backdrop-blur-md dark:bg-slate-900 ${audit.color} transition-all duration-300 group-hover:scale-110`}>
                  ...
                  >
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div className="space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="text-sm font-extrabold text-slate-950 dark:text-slate-50">
                            {log.action}
                          </span>
                          <Badge variant="outline" className="text-[9px] font-black tracking-widest uppercase px-2 py-0.5 rounded-lg border-slate-200 text-slate-500 dark:border-slate-700 dark:text-slate-400">
                            {log.subjectType || "Système"}
                          </Badge>
                        </div>
                        <p className="text-xs font-bold text-slate-400 dark:text-slate-500">
                          Par : <span className="font-extrabold text-slate-700 dark:text-slate-300">{log.user?.name || log.user?.email || "Système"}</span>
                        </p>
                      </div>

                      <div className="text-left sm:text-right space-y-1.5 shrink-0">
                        <p className="text-xs font-mono font-black text-slate-400 dark:text-slate-500">
                          {formatDateTime(log.createdAt)}
                        </p>
                        <div className="flex items-center sm:justify-end gap-2">
                          {log.ipAddress && (
                            <span className="inline-flex items-center gap-1 text-[9px] font-bold text-slate-400 font-mono bg-slate-100 dark:bg-slate-800/50 px-2 py-0.5 rounded-md">
                    ...
                    <AnimatePresence>
                      {isExpanded && (
                        ...
                        >
                          {log.subjectId && (
                            <div className="flex items-center gap-2 rounded-xl bg-slate-100/40 p-2.5 dark:bg-slate-800/30 border border-slate-200/10">
                              <Eye className="h-4 w-4 text-slate-400 shrink-0" />
                              <span className="text-[10px] font-mono text-slate-400 block truncate">
                                ID de l&apos;élément cible : <span className="font-black text-slate-700 dark:text-slate-300">{log.subjectId}</span>
                              </span>
                            </div>
                          )}
                          ...
                          {log.userAgent && (
                            <div className="flex items-center gap-2 rounded-xl bg-slate-100/40 p-2.5 dark:bg-slate-800/30 border border-slate-200/10">
                              ...
                              <span className="text-[10px] font-mono text-slate-400 block truncate">
                                Agent utilisateur : <span className="font-black text-slate-700 dark:text-slate-300">{log.userAgent}</span>
                              </span>
                            </div>
                          )}

                          {log.changes != null && (
                            <div className="rounded-2xl border border-slate-200/40 bg-slate-950 p-4 font-mono text-xs text-slate-200 shadow-inner">
                              <div className="flex items-center gap-2 text-[9px] font-black text-slate-500 uppercase tracking-wider mb-2 border-b border-slate-900 pb-1.5">
