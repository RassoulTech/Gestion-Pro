...
export default function CGUPage() {
  ...
  return (
    <div className="relative pt-32 pb-24 overflow-hidden">
      {/* Decorative Blur */}
      <div className="absolute top-1/4 left-10 w-[300px] h-[300px] bg-orange-600/5 blur-[100px] rounded-full pointer-events-none" />
          ...
          >
            <div className="p-6 rounded-3xl bg-white/40 dark:bg-zinc-900/40 border border-zinc-200/50 dark:border-zinc-800/50 backdrop-blur-xl shadow-sm space-y-4">
              <h3 className="text-xs font-black uppercase tracking-widest text-zinc-900 dark:text-zinc-50 flex items-center gap-2">
                <FileText className="h-4 w-4 text-orange-600" /> Table des matières
              </h3>
                ...
                {sections.map((sec) => (
                  <li key={sec.id}>
                    <button
                      ...
                      className="text-left text-xs font-bold text-zinc-500 hover:text-orange-600 dark:text-zinc-400 dark:hover:text-orange-400 transition-colors py-1 flex items-center gap-1.5 group"
                    >
                      <ArrowRight className="h-3 w-3 opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all text-orange-500" />
                      {sec.title}
                    </button>
                  </li>
                ))}
