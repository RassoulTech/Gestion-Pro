...
const categories = [
  { id: "getting-started", icon: Settings },
  { id: "stock", icon: BookOpen },
  { id: "billing", icon: CreditCard },
  { id: "eboutique", icon: ShoppingBag },
];

type SupportFaq = { category: string; q: string; a: string };

export default function SupportPage() {
  const t = useTranslations("public.support");
  const faqs = t.raw("faqs") as SupportFaq[];
        ...
        <div className="text-center max-w-2xl mx-auto space-y-6">
          ...
          >
            <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-orange-500/10 text-orange-600 dark:text-orange-400 text-xs font-black uppercase tracking-widest border border-orange-500/20">
              <HelpCircle className="h-3 w-3" /> {t("eyebrow")}
            </span>
          </motion.div>
          ...
          >
            {t("titleLead")} <br />
            <span className="bg-gradient-to-r from-orange-600 to-emerald-500 bg-clip-text text-transparent">{t("titleHighlight")}</span>
          </motion.h1>
            ...
            {categories.map((cat, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs font-black uppercase tracking-wider">{t(`categories.${cat.id}`)}</span>
                </motion.button>
              );
            })}
          </div>
          ...
          <AnimatePresence mode="wait">
            {filteredFaqs.length > 0 ? (
              ...
              <div className="text-center py-16 space-y-4">
                ...
                <h3 className="text-lg font-black text-zinc-900 dark:text-zinc-50">{t("noResultsTitle")}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto pt-8">
          <div className="p-8 rounded-3xl bg-gradient-to-tr from-emerald-600/10 to-emerald-500/10 border border-emerald-500/20 backdrop-blur-xl flex flex-col justify-between space-y-6">
            <div className="space-y-3">
              ...
              <h3 className="text-lg font-black text-zinc-900 dark:text-zinc-50">{t("whatsappTitle")}</h3>
          ...
          <div className="p-8 rounded-3xl bg-gradient-to-tr from-orange-600/10 to-orange-500/10 border border-orange-500/20 backdrop-blur-xl flex flex-col justify-between space-y-6">
            <div className="space-y-3">
              ...
              <h3 className="text-lg font-black text-zinc-900 dark:text-zinc-50">{t("callTitle")}</h3>
              <p className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 leading-relaxed">
                {t("callDesc")}
              </p>
            </div>
